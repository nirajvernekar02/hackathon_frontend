...
const DetailsTable = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:3500/api/budget/debts/${id}`, {
        headers: {
...
//   };
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await axios.post(`http://localhost:3500/api/budget/debts/${id}`, expenseForm, {
        headers: {
          Authorization: `Bearer ${authtoken}`
        }
      });
  ...
  const handleDeleteExpense = async (index) => {
    try {
      ...
      const response = await axios.delete(`http://localhost:3500/api/budget/debts/${id}/${expenses[index]._id}`, {
        headers: {
          Authorization: `Bearer ${authtoken}`
        }
      });
      ...
      <div className='m-6'>
        ...
        <TableContainer component={Paper} sx={{ width: '100%', overflowX: 'auto' }} >
          <Table>
            <TableHead>
              <TableRow>
                <TableCell sx={{ fontWeight: 'bold' }}>Creditor</TableCell>
                <TableCell sx={{ fontWeight: 'bold' }}>Amount</TableCell>
                <TableCell sx={{ fontWeight: 'bold' }}>DueDate</TableCell>
            ...
            <TableBody>
              {expenses.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((expense, index) => (
                <TableRow key={index}>
                  <TableCell>{expense.creditor}</TableCell>
