...
//   };
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await axios.post(`http://localhost:3500/api/category/add-category`, expenseForm, {
        headers: {
          Authorization: `Bearer ${authtoken}`
        }
      });
  ...
  const handleDeleteExpense = async (index) => {
    try {
      ...
      const response = await axios.delete(`http://localhost:3500/api/category/delete-category/${expenses[index]._id}`, {
        headers: {
          Authorization: `Bearer ${authtoken}`
        }
      });
